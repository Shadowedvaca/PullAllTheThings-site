...
// ---------------------------------------------------------------------------
// Roster Needs (Phase 1E.1)
// ---------------------------------------------------------------------------

// Collapsed instance state: Set of instance names that are currently collapsed.
// Instances start expanded by default.
const _needsCollapsed = new Set();

function _needsCellHtml(trackData) {
    // Returns the HTML for a needs cell given track data (or null for empty).
    if (!trackData) return '<span class="gp-needs-empty">·</span>';
    const pc = trackData.player_count;
    const ic = trackData.item_count;
    let cls = 'gp-needs-cell--green';
    if (pc >= 6) cls = 'gp-needs-cell--red';
    else if (pc >= 3) cls = 'gp-needs-cell--gold';
    return `<span class="gp-needs-cell ${cls}" title="${pc} player${pc !== 1 ? 's' : ''}, ${ic} item slot${ic !== 1 ? 's' : ''}">${pc}&nbsp;|&nbsp;${ic}</span>`;
}

function _needsTrackTh(track, color) {
    return `<th class="gp-needs-th--${track}" style="color:${color}" data-track="${track}">${track}</th>`;
}

function renderRaidNeedsTable(instances) {
    const container = document.getElementById('gp-needs-raid-container');
    if (!container) return;

    if (!instances || instances.length === 0) {
        container.innerHTML = '<p style="color:var(--color-text-muted); font-size:0.85rem; margin:0;">No raid needs found. Make sure players have active gear plans with items set.</p>';
        return;
    }

    // Determine which tracks have any data across all bosses
    const RAID_TRACKS = ['V', 'C', 'H', 'M'];
    const TRACK_COLORS = { V: '#1eff00', C: '#0070dd', H: '#a335ee', M: '#ff8000' };
    const activeTrackSet = new Set();
    for (const inst of instances) {
        for (const boss of inst.bosses) {
            for (const t of Object.keys(boss.tracks)) {
                activeTrackSet.add(t);
            }
        }
    }
    const activeTracks = RAID_TRACKS.filter(t => activeTrackSet.has(t));

    let html = '<table class="gp-needs-table"><thead><tr>';
    html += '<th class="gp-needs-th--name">Instance / Boss</th>';
    for (const t of activeTracks) {
        html += _needsTrackTh(t, TRACK_COLORS[t]);
    }
    html += '</tr></thead><tbody>';

    for (const inst of instances) {
        const isCollapsed = _needsCollapsed.has(inst.name);
        const collapseIcon = isCollapsed ? '▶' : '▼';
        html += `<tr class="gp-needs-instance" onclick="_toggleNeedsInstance(this, '${inst.name.replace(/'/g, "\\'")}')">`;
        html += `<td class="gp-needs-td--name">${collapseIcon} ${inst.name}</td>`;
        for (const t of activeTracks) {
            const roll = inst.rollup[t];
            html += `<td>${_needsCellHtml(roll)}</td>`;
        }
        html += '</tr>';

        if (!isCollapsed) {
            for (const boss of inst.bosses) {
                html += '<tr class="gp-needs-boss">';
                html += `<td class="gp-needs-td--name">${boss.name}</td>`;
                for (const t of activeTracks) {
                    html += `<td>${_needsCellHtml(boss.tracks[t])}</td>`;
                }
                html += '</tr>';
            }
        }
...
function _toggleNeedsInstance(row, instanceName) {
    if (_needsCollapsed.has(instanceName)) {
        _needsCollapsed.delete(instanceName);
    } else {
        _needsCollapsed.add(instanceName);
    }
    // Re-render with same cached data
    if (window._cachedRaidNeeds) renderRaidNeedsTable(window._cachedRaidNeeds);
}

function renderDungeonNeedsTable(dungeons) {
    const container = document.getElementById('gp-needs-dungeon-container');
    if (!container) return;

    if (!dungeons || dungeons.length === 0) {
        container.innerHTML = '<p style="color:var(--color-text-muted); font-size:0.85rem; margin:0;">No M+ needs found. Make sure players have active gear plans with M+ dungeon items set.</p>';
        return;
    }

    const DUNGEON_TRACKS = ['C', 'H'];
    const TRACK_COLORS = { C: '#0070dd', H: '#a335ee' };
    const activeTrackSet = new Set();
    for (const d of dungeons) {
        for (const t of Object.keys(d.tracks)) activeTrackSet.add(t);
    }
    const activeTracks = DUNGEON_TRACKS.filter(t => activeTrackSet.has(t));

    let html = '<table class="gp-needs-table"><thead><tr>';
    html += '<th class="gp-needs-th--name">Dungeon</th>';
    for (const t of activeTracks) {
        html += _needsTrackTh(t, TRACK_COLORS[t]);
    }
    html += '</tr></thead><tbody>';
    ...
    for (const d of dungeons) {
        html += '<tr>';
        html += `<td class="gp-needs-td--name">${d.name}</td>`;
        for (const t of activeTracks) {
            html += `<td>${_needsCellHtml(d.tracks[t])}</td>`;
        }
        html += '</tr>';
    }
...
async function loadRosterNeeds() {
    const includeInitiates = document.getElementById('needs-initiates')?.checked ?? true;
    const includeOffspec   = document.getElementById('needs-offspec')?.checked ?? false;
    const params = `?include_initiates=${includeInitiates}&include_offspec=${includeOffspec}`;

    const raidEl   = document.getElementById('gp-needs-raid-container');
    const dungEl   = document.getElementById('gp-needs-dungeon-container');
    if (raidEl) raidEl.innerHTML = '<div class="gp-needs-loading"><span class="spinner"></span> Loading…</div>';
    if (dungEl) dungEl.innerHTML = '<div class="gp-needs-loading"><span class="spinner"></span> Loading…</div>';
    ...
    try {
        ...
        if (!raidResp.ok) throw new Error(raidData.detail || 'Raid needs fetch failed');
        if (!dungResp.ok) throw new Error(dungData.detail || 'Dungeon needs fetch failed');

        window._cachedRaidNeeds = raidData.instances || [];
        renderRaidNeedsTable(window._cachedRaidNeeds);
        renderDungeonNeedsTable(dungData.dungeons || []);
    } catch (err) {
        const errHtml = `<p style="color:#f87171; font-size:0.85rem; margin:0;">Error: ${err.message}</p>`;
        if (raidEl) raidEl.innerHTML = errHtml;
        if (dungEl) dungEl.innerHTML = errHtml;
    }
}
