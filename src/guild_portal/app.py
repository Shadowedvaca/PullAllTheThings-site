"""Guild platform application factory."""
...
async def _run_campaign_checker(database_url: str) -> None:
    ...
    from guild_portal.services.campaign_service import check_campaign_statuses
...
async def _run_contest_agent(database_url: str) -> None:
    ...
    from guild_portal.services.contest_agent import run_contest_agent
...
async def _auto_book_loop(pool: asyncpg.Pool) -> None:
    ...
    from guild_portal.services.raid_booking_service import check_and_auto_book, POLL_INTERVAL_SECONDS
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        logger.info("Starting guild platform (env=%s)", settings.app_env)
        ...
        if guild_sync_pool is not None:
            try:
                async with guild_sync_pool.acquire() as _sc_conn:
                    sc_row = await _sc_conn.fetchrow(
                        "SELECT * FROM common.site_config LIMIT 1"
                    )
                if sc_row:
                    set_site_config(dict(sc_row))
                    logger.info("Site config loaded: guild=%s", sc_row["guild_name"])
                else:
                    logger.warning("No site_config row found — using defaults")
            except Exception as exc:
    ...
    from guild_portal.templating import templates as _templates
    ...
    # Register API routes
    from guild_portal.api.health import router as health_router
    ...
    from guild_portal.pages.vote_pages import router as vote_page_router
