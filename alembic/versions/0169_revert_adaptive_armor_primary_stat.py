"""revert: restore primary_stat from payload for rows nulled by migration 0168

Migration 0168 incorrectly NULLed primary_stat on all unrestricted non-weapon
items.  This revert re-derives primary_stat from landing.blizzard_items payload
for exactly those rows, then restores sp_rebuild_items to the pre-0168 version.
...
def upgrade() -> None:
    # ── 1. Restore primary_stat from payload for the rows 0168 NULLed ─────────
    op.execute("""
        UPDATE enrichment.items ei
           SET primary_stat = COALESCE(
         ...
         WHERE ei.blizzard_item_id = bi.blizzard_item_id
           AND ei.primary_stat IS NULL
           AND ei.slot_type NOT IN ('one_hand', 'two_hand', 'ranged', 'off_hand')
           AND ei.playable_class_ids IS NULL
    """)

    # ── 2. Revert sp_rebuild_items to pre-0168 version (no adaptive nulling) ──
    op.execute("""
