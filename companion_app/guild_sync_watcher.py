...
API_URL = os.getenv("GUILD_SYNC_API_URL", "")
if not API_URL:
    logging.basicConfig(level=logging.ERROR)
    logging.error("GUILD_SYNC_API_URL is not set. Set it in your .env file (e.g. GUILD_SYNC_API_URL=https://yoursite.com/api)")
    sys.exit(1)
API_KEY = os.getenv("GUILD_SYNC_API_KEY", "")
WOW_SV_PATH = os.getenv("WOW_SAVED_VARIABLES_PATH", "")
POLL_INTERVAL = int(os.getenv("POLL_INTERVAL", "30"))
DEBUG = os.getenv("DEBUG", "false").lower() == "true"

# File to watch
TARGET_FILE = "GuildSync.lua"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("guild_sync_watcher.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("GuildSync")
...
class LuaParser:
    ...
    def parse_file(filepath: str) -> dict:
        ...
        # Find the main assignment: GuildSyncDB = { ... }
        match = re.search(r"GuildSyncDB\s*=\s*(\{.*\})\s*$", content, re.DOTALL)
        if not match:
            raise ValueError("Could not find GuildSyncDB table in file")
...
class SyncWatcher(FileSystemEventHandler):
    """Watches for changes to the GuildSync SavedVariables file."""
    ...
    def _upload(self, payload: dict) -> bool:
        """Upload parsed data to the guild API. Returns True on success."""
...
def validate_config():
    ...
    if not API_URL:
        errors.append("GUILD_SYNC_API_URL is not set")
    if not API_KEY:
        errors.append("GUILD_SYNC_API_KEY is not set")
    if not WOW_SV_PATH:
...
def main():
    """Main entry point."""
    print("=" * 50)
    print("  Guild Sync Companion App")
