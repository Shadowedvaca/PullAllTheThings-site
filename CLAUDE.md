...
### Current Phase
- **Archon BIS Extraction — Phase D COMPLETE** (no migration, dev only, branch `feature/archon-bis-extraction`). `_archon_source_ts_from_raw()` + `_archon_is_unchanged()` pure helpers in `bis_sync.py`; `sync_target()` queries `MAX(source_updated_at)` from `landing.bis_scrape_raw` for archon targets — sets `status='skipped'` and skips landing insert when `page.lastUpdated` is unchanged. `run_archon_sync()` docstring already referenced this; now fully wired. 12 new tests; 1702/1708 suite-wide (6 pre-existing failures unchanged). **Next: PR → main → prod.**
