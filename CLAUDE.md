...
### Current Phase
- **BIS Note & Guide Folding — Phase 5 COMPLETE** (no migration, on dev, branch `feature/iv-bis-extraction`). `_iv_classify_tab_label` extended with optional `raid_instance_names: frozenset[str]` param. When provided, tab labels containing a known raid instance name from `landing.blizzard_journal_instances` (instance_type='raid') are classified as `'raid'` even when the word "raid" is absent — and this check fires before the generic "bis"/"overall" fallback so labels like "Liberation of Undermine BiS List" aren't misclassified as overall. New `_load_raid_instance_names(conn)` helper. Instance names threaded through `_iv_parse_sections` → `_iv_parse_from_image_blocks` → `_iv_classify_tab_label`, loaded in `rebuild_bis_from_landing` (both passes) and `_extract_icy_veins`. 12 new tests in `tests/unit/test_bis_sync_iv.py`; 1648/1654 suite-wide. Deployed to dev. **Next planned:** PR → main → prod.
...
- **Next planned:** PR → main → prod (all 5 phases on feature/iv-bis-extraction complete)
