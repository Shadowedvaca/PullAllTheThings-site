...
### Current Phase
- **IV BIS Extraction — Phase Z.2 COMPLETE** (on dev, no migration, branch `feature/iv-bis-extraction`).
  ...
  - **Z.2** (no migration): `_extract_icy_veins()` rewrite + dead code removal. Removed: `discover_iv_areas()`, `_fetch_iv_areas()`, `_IV_AREA_LINK_RE`, `_HTML_TAG_RE`, `_categorize_iv_area()`, `_IV_ITEM_*_RE` regexes, old stub. Added: `IVSection` dataclass, `_iv_classify_section()`, `_iv_extract_regular_rows()`, `_iv_extract_trinket_rows()`, `_iv_is_outlier()`, `_iv_parse_sections()`, `_upsert_iv_sections()`, new `_extract_icy_veins(url, content_type, spec_id, source_id, pool)`. Removed IV skip blocks from `sync_source()` and `sync_target()`. Added `source_id` to `_extract()` dispatcher. 1550 unit tests pass (88 new IV tests in `test_bis_sync_iv.py`).
  - **Z.2b** (no migration): image_block tab parsing. Added `_iv_classify_tab_label()` and `_iv_parse_from_image_blocks()`. IV pages use `div.image_block_header_buttons` with `span[id="area_N_button"]` tab labels — more reliable than h3_id matching. image_block path is now primary; heading_container path is fallback for test fixtures. Fixes 12 failing specs: 7 had stale h3_id classifier results (need Re-Sync Errors), 5 had no h3 at all (Blood DK, Marksmanship, Protection, Survival, Vengeance). 1565 unit tests pass (47 IV tests).
  - **Next:** Z.2.5 — unified `landing.bis_page_sections` + `config.bis_section_overrides` (migration 0162); replaces `landing.method_page_sections`, `landing.iv_page_sections`, `config.method_section_overrides`. Override support enabled for IV (needed for Blood DK 4-tab layout). Then Z.3 — unified Section Inventory admin UI.
...
- **Next planned:** Z.2.5 — unified bis_page_sections table (migration 0162), then Z.3 — unified Section Inventory admin UI
