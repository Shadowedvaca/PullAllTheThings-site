...
### Current Phase
- **Gear Plan Phase 1A** (in dev, unreleased) — Foundation: migration 0066 creates 10 new tables (`wow_items`, `item_sources`, `hero_talents`, `bis_list_sources`, `bis_list_entries`, `character_equipment`, `gear_plans`, `gear_plan_slots`, `bis_scrape_targets`, `bis_scrape_log`) plus `last_equipment_sync` column on `wow_characters`. New modules: `quality_track.py` (V/C/H/M detection from Blizzard display_string + SimC bonus IDs), `equipment_sync.py` (per-slot gear sync, last-login-filtered batch), `item_service.py` (Wowhead tooltip API fetch + `wow_items` cache). `BlizzardClient.get_character_equipment()` returns full `CharacterEquipmentSlot` list. Scheduler gains Step 8: equipment sync (non-fatal). ORM models added for all 10 tables. `bis_list_sources` seeded with 5 rows (Archon Raid/M+, Wowhead Overall, Icy Veins Raid/M+). 28 new unit tests (quality track parsing + slot normalisation). No UI yet — pure backend foundation.
- **Branch:** `main`
- **Tests:** 1107 pass (28 new; 2 pre-existing bnet template failures unchanged)
- **Last migration:** 0066
- **Last tag:** `prod-v0.10.0` (Gear Plan not yet tagged to prod)
- **Active branch:** `main`
- **Next:** Phase 1B — BIS discovery + extraction pipeline (`bis_sync.py`, `simc_parser.py`, admin BIS sync dashboard)
