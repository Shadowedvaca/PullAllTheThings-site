...
- Phase 4.4.3: Onboarding Activation & OAuth Integration (no migration) — on_member_join wired with enable_onboarding feature flag; conversation._auto_provision() transitions to oauth_pending (not provisioned), sends OAuth prompt DM, starts 10-min polling for oauth_complete; update_onboarding_status() module-level function signals oauth_complete from bnet_auth_routes callback; deadline_checker handles oauth_pending → 24h reminder → 48h abandoned_oauth; /resend-oauth admin slash command; set_app_url/get_app_url in config_cache. 528 tests pass, 69 skip.

### Current Phase
- **Platform is feature-complete through Phase 4.4.3.** Next planned: Phase 4.4.4 Data Quality Simplification (see `reference/` for plans). Phases 4.5 (Warcraft Logs) and 4.6 (AH Pricing) deferred.

### Recent Changes (Phase 4.4.3, 2026-03-14, no migration)
- **Phase 4.4.3 complete**: Onboarding Activation & OAuth Integration. `enable_onboarding` flag added to `SiteConfig` model and `is_onboarding_enabled()` in `config_cache.py`; checked in `bot.py on_member_join` before starting conversation. `set_app_url`/`get_app_url` added to `config_cache.py`, populated at startup from `settings.app_url`. `conversation._auto_provision()`: state now `oauth_pending` (not `provisioned`); sends OAuth prompt DM; starts `_poll_for_oauth_complete()` (10 × 60s). Module-level `update_onboarding_status(pool, player_id, new_status)`: updates `oauth_pending` session for a player — called from `bnet_auth_routes` callback after `sync_bnet_characters()`. `deadline_checker._provision()` → `oauth_pending`; new `_check_oauth_pending_sessions()`: 24h → `_send_oauth_reminder()`, 48h → `_abandon_oauth()` with friendly DM. `commands.py`: `/resend-oauth {member}` command added; NOT IN lists updated to include `oauth_complete`/`abandoned_oauth`. 528 tests pass, 69 skip.
