...
- **Drift scanner**: `detect_note_mismatch` and `detect_link_note_contradictions` in `integrity_checker.py` now skip links with `link_source = 'battlenet_oauth'`. OAuth ownership is authoritative — no drift check needed.
