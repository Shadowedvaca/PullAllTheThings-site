...
### Current Phase
- **Phase UI-1C (complete)** — Paperdoll redesign on `/my-characters-new`. Two-box slot cards (upgrade box + equipped box) in left/right columns. Gear tab fully wired: Hero Talent + BIS Source selects, Sync Gear, Fill BIS, Import/Export SimC, Reset Plan, slot drawer with BIS grid + lock/unlock. Layout deviations from spec: shirt/tabard removed entirely; both Main Hand and Off Hand in left column (bottom) with a faint separator rule between Wrist and Main Hand. Phases UI-1A and UI-1B also complete.
...
- **Next:** Phase UI-1D — Gear detail Option C table (full-width slot table below the paperdoll in the Gear panel). No API changes; same `get_plan_detail` endpoint.
