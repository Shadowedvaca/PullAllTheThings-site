...
Three PostgreSQL schemas, current through **migration 0039**:
...
- Phase 4.5: Warcraft Logs Integration (migration 0039) — `guild_identity.wcl_config/character_parses/raid_reports`; `warcraftlogs_client.py` (OAuth2 + GraphQL); `wcl_sync.py` (report + parse sync, attendance); scheduler daily 5 AM pipeline; `/admin/warcraft-logs` page (config, sync status, reports, attendance grid, top parses); `GET /api/v1/guild/parses` public endpoint. 539 tests pass, 69 skip.
...
### Current Phase
- **Platform is feature-complete through Phase 4.5.** Phase 4.6 (AH Pricing) deferred. All Phase 4 work deploys to prod together when complete.

### Recent Changes (Phase 4.5, 2026-03-15, migration 0039)
- **Phase 4.5 complete**: Warcraft Logs Integration.
- **Migration 0039**: 3 new tables in `guild_identity` schema — `wcl_config` (single-row, same pattern as crafting_sync_config), `character_parses` (best WCL percentile per char/encounter/difficulty/spec), `raid_reports` (guild reports with attendee JSONB). `screen_permission` for `warcraft_logs` added.
...
- **`wcl_sync.py`**: `load_wcl_config()`, `sync_guild_reports()` (skips existing, fetches fight details + attendees for new reports), `sync_character_parses()` (batched, 2s/batch rate throttle, upserts best-per-encounter), `_parse_zone_rankings()` (extracts per-boss parses from WCL response), `compute_attendance()` (attendance rates from stored reports).
- **Scheduler**: `run_wcl_sync()` added — daily at 5 AM UTC; loads + decrypts config from DB; runs report sync then character parse sync; updates `wcl_config.last_sync*`; non-fatal.
- **ORM models**: `WclConfig`, `CharacterParse`, `RaidReport` added to `models.py`.
- **Admin page**: `GET /admin/warcraft-logs` — Configuration card (client ID/secret, guild/server/region, verify + save), Sync Status card (last sync, force sync button), Recent Reports table (date/title/zone/kills/attendance/link), Attendance Grid (last 10 reports × players), Top Parses table (character/encounter/difficulty/spec/percentile/log link).
- **Admin API**: `GET /admin/warcraft-logs/config`, `PATCH /admin/warcraft-logs/config` (encrypts secret with JWT key), `POST /admin/warcraft-logs/verify`, `POST /admin/warcraft-logs/trigger`, `GET /admin/warcraft-logs/reports`, `GET /admin/warcraft-logs/attendance`, `GET /admin/warcraft-logs/parses`.
