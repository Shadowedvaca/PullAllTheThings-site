...
### Current Phase
- **Phase UI-1E (complete)** — Raid and M+ detail panels on `/my-characters-new`. Raid panel: difficulty tabs (Normal/Heroic/Mythic) with killed/total counts; per-boss list with ✓/✗ indicators; filters to active season via `current_raid_ids`. M+ panel: color-coded overall score + per-dungeon table (key level, timed indicator, score); filtered to active season via `blizzard_mplus_season_id`. Both panels load from `GET /api/v1/me/character/{id}/progression` (endpoint extended to return `raid_bosses` per-boss detail and `mythic_plus.dungeons` per-dungeon breakdown). RIO / WCL / Armory external profile links moved to the persistent guides bar (visible in all panel states), separated from guide badges by a subtle border rule. Phases UI-1A–1D also complete.
...
- **Next:** Phase UI-1F — Parses detail panel.
