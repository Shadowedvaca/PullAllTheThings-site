...
### Current Phase
- **Phase UI-1A (complete)** — Foundation for unified `/my-characters-new` page (UI redesign). Migration 0080 (`race VARCHAR(40)` on `wow_characters`). New route `/my-characters-new`, new template/CSS/JS (`my_characters_new.*`). `/api/v1/me/characters` now returns `race` + `role` per character. Header redesigned: centered page-title style (name gold Cinzel, realm/BNet badge, class+spec+race+role subtitle). Character selector top-right corner; guide badge links (colored, click-to-open) below header with underline-only spec dropdown. Old `/my-characters` and `/gear-plan` untouched.
...
- **Next:** Phase UI-1B — summary cards (6-card grid) + center panel switching. Needs new `GET /api/v1/me/character/{id}/summary` endpoint.
