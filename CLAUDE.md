...
- **Phase 1D.5 complete** — Tier token pipeline.
  ...
  - `process_tier_tokens(pool)` in `item_source_sync.py`: 3 steps — (1) detects tier tokens from wowhead_tooltip_html ('Synthesize a soulbound set'); parses slot (hand→hands normalised), eligible_class_ids from Wowhead Classes div, armor_type from class IDs; upserts into tier_token_attrs (skips is_manual_override=TRUE rows); (2) backfills `wow_items.armor_type` for tier piece items from tooltip HTML via `_armor_type_from_tooltip()` — parses `>(Cloth|Leather|Mail|Plate)<` — required so the view join `tp.armor_type = tta.armor_type` resolves correctly (Wowhead `jsonequip.subclass` does not populate this field); (3) calls `flag_junk_sources(flag_tier_pieces=True)`. Returns `tier_pieces_armor_type_updated` count. On Midnight S1: 21 tokens processed, 149 tier pieces updated, 538 junk sources flagged. Each tier piece now shows exactly 2 bosses: the slot-specific boss (e.g. Lightblinded Vanguard for head) + Midnight Falls (Chiming Void Curio / any-slot any-armor token).
  ...
  - 38 unit tests in `test_tier_token_processor.py` (includes `_armor_type_from_tooltip` tests + backfill smoke test); 1311 total pass.
  - **Workflow note:** must run in order after each Sync Loot Tables — Sync Loot Tables → Enrich Items → Process Tier Tokens. Phase 1D.6 surfaces this as a labeled 5-step workflow in the admin UI.
...
- **Process Tier Tokens must re-run after each Sync Loot Tables** — `enrich_catalyst_tier_items()` adds broad per-boss source rows for tier pieces after every "Sync Loot Tables". Those rows are unflagged until "Process Tier Tokens" runs again and calls `flag_junk_sources(flag_tier_pieces=True)`. Correct workflow: Sync Loot Tables → Enrich Items → Process Tier Tokens. Phase 1D.6 will make this order explicit in the admin UI.
