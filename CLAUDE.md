...
- **Last migration:** 0140 (on prod); **0146 on dev** (feature branch — all Phase E migrations deployed)
- **Last prod tag:** `prod-v0.20.2`
- **Active branch:** `feature/gear-plan-1.0-wow_items-fix`
- **In progress:** `wow_items` retirement — plan at `reference/gear-plan-1.0-wow_items-fix.md`. ALL PHASES A–E COMPLETE on dev (migrations 0141–0146). `guild_identity.wow_items` dropped, `v_tier_piece_sources` dropped, `tier_token_attrs.blizzard_item_id` is now the PK, all integer FK columns removed. Prod snapshot: `/opt/guild-portal/backups/patt_db_pre_phase_e_20260418_141749.dump` (142MB). Ready for PR → main → prod-v0.21.0.
