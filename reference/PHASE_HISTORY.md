...
- **Hit's Wheel of Fate (migration 0181):** Member `/spec-wheel` page with equal-chance main/off-spec spinner, seasonal first/latest history and roll count, roster-aware filters excluding initiates, replacement confirmation, and optional matching-class character assignment. Follow-up added shared hamburger navigation, assignment-state refresh, public roster first/latest wheel results, and color-coded first/latest notes beside My Profile character selectors.
