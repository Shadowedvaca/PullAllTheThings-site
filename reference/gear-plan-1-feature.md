...
| **1D.4** Loot table junk flagging | ✅ COMPLETE | Migration 0086: `is_suspected_junk` on `item_sources`; `flag_junk_sources()` flags null-ID world boss rows + tier piece direct-source rows; `get_item_sources()`/`get_instance_names()` exclude junk by default (show_junk param); gear_plan_service filters junk; admin Item Sources gets Show Junk toggle + Flag Junk Sources button (GL); junk rows dimmed with red badge. |
| **1D.5** Tier token pipeline | ⬜ TODO | `tier_token_attrs` table; `process_tier_tokens()` post-processor (also calls `flag_junk_sources()`); `v_tier_piece_sources` view; gear plan service uses view for tier pieces. Migration 0087. |
